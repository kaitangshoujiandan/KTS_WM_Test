      ...
      >
        ...
        <el-table-column label="操作" width="140">
          <template #default="scope">
            ...
            <el-popconfirm title="确定删除这个商品分类吗？" @confirm="del(scope.row.id)">
              <template #reference>
...
const del = (id) => {
  proxy.$axios.post(proxy.$baseURL + '/goodsType/delete', { id }).then((res) => {
    if (res.data.code == 1) {
...
const save = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      let url = form.value.id ? '/goodsType/update' : '/goodsType/save'
...
const loadPost = () => {
  proxy.$axios
    .post(proxy.$baseURL + '/goodsType/listpage', {
